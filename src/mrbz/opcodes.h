 ...
 * Opcode definitions (from mruby bytecode format)
 *
 * Based on mruby/c opcodes (BSD 3-Clause License)
 * Copyright (C) 2015- Kyushu Institute of Technology.
 */

#ifndef MRBZ_OPCODES_H
#define MRBZ_OPCODES_H
...
enum OPCODE {
    ...
    // Method calls
    OP_SSEND      = 0x2D, // BBB  R[a] = self.send(Syms[b],R[a+1]..)
    OP_SSENDB     = 0x2E, // BBB  R[a] = self.send(Syms[b],..,&R[..])
    OP_SEND       = 0x2F, // BBB  R[a] = R[a].send(Syms[b],R[a+1]..)
    OP_SENDB      = 0x30, // BBB  R[a] = R[a].send(Syms[b],..,&R[..])
    OP_CALL       = 0x31, // Z    R[0] = self.call(frame.argc, frame.argv)
    OP_SUPER      = 0x32, // BB   R[a] = super(R[a+1],... ,R[a+b+1])
    OP_ARGARY     = 0x33, // BS   R[a] = argument array
    OP_ENTER      = 0x34, // W    arg setup according to flags
